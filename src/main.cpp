...
#if defined(NDEBUG)
# error "Newcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6ab6322bd4581408312ec89053116b48e730c42739e0f5f3c84a5f5fc5cf16f7");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Newcoin: starting difficulty is 1 / 2^12
...
{
    // Newcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Newcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Newcoin: 1 days
static const int64 nTargetSpacing = 5 * 60; // Newcoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Newcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Newcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x0b172779e9601178df32914433d5dd01798734dcbb64a20440160aa46c24f56a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1522082192;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084525883;

        if (fTestNet)
        {
            block.nTime    = 1522082176;
            block.nNonce   = 385735212;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
{
    ...
    printf("block.GetHash = %s\n", block.GetHash().ToString().c_str());
        ...
        assert(block.hashMerkleRoot == uint256("0x66a40994ff8f94acc73f0d4acbe82ccd8c375f005745a3537c5f2cb936125502"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Newcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NewcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NewcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NewcoinMiner(CWallet *pwallet)
{
    printf("NewcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("newcoin-miner");
    ...
    try { loop {
        ...
        printf("Running NewcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NewcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NewcoinMiner, pwallet));
}
